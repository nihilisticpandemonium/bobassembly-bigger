...
if mods["MilesBobsExpansion"] then
	...
	for tier = 7, 10 do
		local name = "assembling-machine-"..tier.."-bigger"
		data:extend({
			{
				type = "item",
				name = name,
				icon = "__MilesBobsExpansion__/graphics/icons/assembling-machine-" .. tier .. ".png",
				icon_size = 32,
				subgroup = "bob-assembly-machine",
				order = "c["..name.."]",
				place_result = name,
			...
			},
			{
				...
				name = name,
				enabled = "false",
				ingredients = ingredientLists[tier],
				result = name
			}
		})
	end
